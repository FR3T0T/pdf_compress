/**
 * Ported verbatim from web/js/icons.js (24x24 stroke-style icon set). This
 * is the ONE canonical icon library for the React app — it replaces both
 * icons.js (used inside page content) and app.js's separate, simpler
 * 20x20 sidebar-only _svgIcon() set, so there's a single source of truth
 * instead of two divergent icon libraries.
 */
