...
from PySide6.QtWidgets import (QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QFrame, QFileDialog, QProgressBar, QComboBox)
from pdf_ops import nup_layout
...
class NupPage(BasePage):
    page_title = "N-up Layout"
    page_icon = "grid"
    page_key = "nup"
    ...
    def _build(self):
        ...
        self.drop_zone.hint_lbl.setText("arrange multiple pages per sheet")
        ...
        row1 = QHBoxLayout()
        row1.addWidget(QLabel("Pages per sheet:"))
        self.nup_combo = QComboBox()
        self.nup_combo.addItems(["2", "4", "8", "9", "16"])
        self.nup_combo.setCurrentText("4")
        row1.addWidget(self.nup_combo, 1)
        root.addLayout(row1)
        root.addSpacing(4)

        row2 = QHBoxLayout()
        row2.addWidget(QLabel("Output page size:"))
        self.size_combo = QComboBox()
        self.size_combo.addItems(["A4", "Letter", "A3"])
        row2.addWidget(self.size_combo, 1)
        root.addLayout(row2)
        root.addSpacing(4)

        row3 = QHBoxLayout()
        row3.addWidget(QLabel("Orientation:"))
        self.orient_combo = QComboBox()
        self.orient_combo.addItems(["Landscape", "Portrait"])
        row3.addWidget(self.orient_combo, 1)
        root.addLayout(row3)
    ...
    def _load_file(self, path):
        ...
        self._output_path = base + "_nup" + ext
    ...
    def _run(self):
        ...
        pages_per_sheet = int(self.nup_combo.currentText())
        page_size = self.size_combo.currentText()
        orientation = self.orient_combo.currentText().lower()

        def _worker():
            try:
                nup_layout(input_path, output, pages_per_sheet, page_size, orientation)
    ...
    def _on_done(self, output):
        ...
        self.result_lbl.setText(f"Layout applied \u2192 {os.path.basename(output)}")
