...
from PySide6.QtWidgets import (QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QFrame, QFileDialog, QProgressBar, QDoubleSpinBox, QComboBox)
from pdf_ops import crop_pages
...
class CropPage(BasePage):
    ...
    def _build(self):
        ...
        self.drop_zone.title_lbl.setText("Select a PDF to crop")
        self.drop_zone.hint_lbl.setText("trim margins from all pages")
        ...
        margin_lbl = QLabel("MARGINS TO TRIM")
        margin_lbl.setFont(QFont(FONT, 8, QFont.Bold))
        root.addWidget(margin_lbl)
        self._margin_lbl = margin_lbl
        root.addSpacing(6)
        ...
        self.unit_combo.addItems(["mm", "inches", "points"])
        unit_row.addWidget(self.unit_combo)
        unit_row.addStretch()
        root.addLayout(unit_row)
        root.addSpacing(6)

        # Margin spinners in a grid
        ...
        self.unit_combo.currentTextChanged.connect(self._update_unit_suffix)
        ...
        self.btn_crop.setObjectName("primary")
        self.btn_crop.setCursor(Qt.PointingHandCursor)
        self.btn_crop.clicked.connect(self._run)
    ...
    def _update_unit_suffix(self, unit):
        suffix = f" {unit}" if unit != "points" else " pt"
        for name in ("left", "right", "top", "bottom"):
            getattr(self, f"spin_{name}").setSuffix(suffix)
    ...
    def _load_file(self, path):
        ...
        self._output_path = base + "_cropped" + ext
    ...
    def _run(self):
        ...
        def _worker():
            try:
                crop_pages(input_path, output, margins, unit)
    ...
    def _on_done(self, output):
        ...
        self.result_lbl.setText(f"Cropped \u2192 {os.path.basename(output)}")
    ...
    def apply_theme(self, theme):
        ...
        self._margin_lbl.setStyleSheet(f"color: {t.text2};")
