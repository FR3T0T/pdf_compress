...
from PySide6.QtWidgets import (QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QFrame, QFileDialog, QProgressBar, QCheckBox)
from pdf_ops import flatten_pdf
...
class FlattenPage(BasePage):
    ...
    def _build(self):
        ...
        self.drop_zone.title_lbl.setText("Select a PDF to flatten")
        self.drop_zone.hint_lbl.setText("remove annotations and form fields")
        ...
        opt_lbl.setFont(QFont(FONT, 8, QFont.Bold))
        root.addWidget(opt_lbl)
        self._opt_lbl = opt_lbl
        root.addSpacing(6)

        self.chk_annotations = QCheckBox("Remove annotations (comments, highlights, stamps)")
        self.chk_annotations.setChecked(True)
        root.addWidget(self.chk_annotations)
        root.addSpacing(2)

        self.chk_forms = QCheckBox("Remove form fields (flatten to static content)")
        ...
        bar = QHBoxLayout()
        bar.addStretch()
        self.btn_flatten = QPushButton("Flatten")
        self.btn_flatten.setObjectName("primary")
        self.btn_flatten.setCursor(Qt.PointingHandCursor)
        self.btn_flatten.clicked.connect(self._run)
        bar.addWidget(self.btn_flatten)
        root.addLayout(bar)
    ...
    def _load_file(self, path):
        ...
        self._output_path = base + "_flattened" + ext
    ...
    def _run(self):
        if self.running or not self._input_path:
            return
        if not self.chk_annotations.isChecked() and not self.chk_forms.isChecked():
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.warning(self, "Nothing to do", "Select at least one option.")
            return
        self.running = True
        ...
        forms = self.chk_forms.isChecked()

        def _worker():
            try:
                flatten_pdf(input_path, output, annotations=annotations, forms=forms)
    ...
    def _on_done(self, output):
        ...
        self.result_lbl.setText(f"Flattened \u2192 {os.path.basename(output)}")
    ...
    def apply_theme(self, theme):
        ...
        self._opt_lbl.setStyleSheet(f"color: {t.text2};")
